...
-Get health data from 
    ...
    -Copy CSV file to /home/cloudera/hive/inpatient

-Get CSV serde jar file from:
 
    https://github.com/downloads/IllyaYalovyy/csv-serde/csv-serde-0.9.1.jar

    -Details on this lib can be found at:

       http://dev.bizo.com/2010/11/csv-and-hive.html

    -Copy jar file to /home/cloudera/hive
...
CREATE EXTERNAL TABLE inpatient (
  ...
  region          STRING,
  discharges      INT,
  covered_charge  STRING, 
  total_charge    STRING,
  medicare_charge STRING)
  COMMENT 'This is inpatient cost data from FY2011 taken from data.gov'
...
LOAD DATA LOCAL INPATH "inpatient/inpatient.csv" INTO TABLE inpatient;

DESCRIBE inpatient;

CREATE EXTERNAL TABLE inpatient_edited(
  procedure        STRING,
  state            STRING,
  total_chargeF    FLOAT)
  COMMENT 'This is an edited version of the inpatient table'
  STORED AS TEXTFILE;

FROM inpatient
INSERT OVERWRITE TABLE inpatient_edited
SELECT procedure, state, float(regexp_replace(substr(total_charge, 2, length(total_charge)), ',', ''));

SELECT procedure, state, total_chargeF FROM inpatient_edited LIMIT 5;

SELECT state, avg(total_chargeF) AS highest_avg_cost FROM inpatient_edited GROUP BY state SORT BY highest_avg_cost DESC;

SELECT procedure, avg(total_chargeF) AS highest_avg_cost FROM inpatient_edited GROUP BY procedure SORT BY highest_avg_cost DESC;
...
Time taken: 101.328 seconds, Fetched: 100 row(s)


Additional Query -- find average for "covered charge"
-----------------------------------------------------
CREATE EXTERNAL TABLE inpatient_edited2(
  procedure        STRING,
  state            STRING, 
  covered_chargeF    FLOAT)
  COMMENT 'This is an edited version of the inpatient table w/ covered charge'
  STORED AS TEXTFILE;

FROM inpatient
INSERT OVERWRITE TABLE inpatient_edited2
SELECT procedure, state, float(regexp_replace(substr(covered_charge, 2, length(covered_charge)), ',', ''));

SELECT procedure, state, covered_chargeF FROM inpatient_edited LIMIT 5;

SELECT state, avg(covered_chargeF) AS highest_avg_cost FROM inpatient_edited2 GROUP BY state SORT BY highest_avg_cost DESC;

SELECT procedure, avg(covered_chargeF) AS highest_avg_cost FROM inpatient_edited2 GROUP BY procedure SORT BY highest_avg_cost DESC;
